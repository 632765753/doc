...
> 工厂模式把创建对象实例的具体细节隐藏，使用者不需要关系对象创建的过程。
...
public class FactoryMain {
	...
	public static void main(String[] args) {
		...
		ISender createSender = factory.createSender("sms");
		createSender.send();
	}
}
```

>  1.存在的问题：每次需要根据参数进行创建不同的实例，如果参数传入错误，就会出现问题 2.假如新加了一个实现类，需要改动工厂方法，破坏了闭包原则。

### 工厂方法模式

> 就是建立一个工厂类，对实现了同一接口的一些类进行实例的创建；可以避免传参错误问题
...
public class Factory {
	...
	public ISender createMailSmsSender() {
		return new SmsSender();
	}
}
...
public class FactoryMain {
	...
	public static void main(String[] args) {
		Factory factory=new Factory();
		ISender createSender = factory.createSmsSender();
		createSender.send();
	}
}
```

> 假如新了一个实现类，需要改动工厂方法，破坏了闭包原则。

### 静态工厂方法模式

> 就是建立一个工厂类，对实现了同一接口的一些类进行实例的创建；可以避免传参错误问题和每次实例化工厂类问题
...
public class Factory {
	...
	public static ISender createMailSmsSender() {
		return new SmsSender();
	}
}
...
public class FactoryMain {
	...
	public static void main(String[] args) {
		...
		ISender createSender = smsFactory.createInstance();
		createSender.send();
	}
}
```
...
## mysql如何实现存在就更新不存在就插入

> on duplicate key update

```sql
insert into student (name,address,id) VALUES ('lisi','zhangsanfgen',9)
ON duplicate KEY UPDATE address='lisi';
```

mysql提供的insert … on duplicate key update的语法，如果insert的数据会引起唯一索引（包括主键索引）的冲突，即这个唯一值重复了，则不会执行insert操作，而执行后面的update操作
...
1.安全性（设置用户名密码、启用ssl加密等）

2.消费消息失败重试几次后的预警（达到多少次后发消息通知相关人员处理）

3.多次消费消息确保幂等性（悲观锁（for update）、乐观锁、唯一约束，状态）
...
```java
public interface CInterface extends AInterface,BInterface,DInterface{
...
2.函数式接口（一个接口只能有一个抽象方法，不包括Object的方法）
...
3.方法引用，构造方法引用（::，方法引用其实是lambda的简写）
...
7.接口可以有default方法和静态方法了

8.将hashmap从数据+链表结构优化成了数组+链表+红黑树

### java.util.stream.Stream的操作

使用流的步骤：数据源(集合)->流(转换成stream)->操作(一系列操作)->结束(转换成新的集合等)
...
> 首先对象要实现`Comparable`接口,并且重写compareTo方法

```java
class Stu implements Comparable<Stu>{
  private Integer age;
  public Stu(Integer age) {
    super();
    this.age = age;
  }
...
public static void main(String[] args) {
  ...
  listS.add(new Stu(121));
  listS.add(new Stu(30));
  listS.add(new Stu(42));
  listS.add(new Stu(1));
  //降序排列 升序照搬 这里就不多写了
...
如果当前指定的下表存在元素，就可以一直删；如果当前不存在这个下标，就会报下标越界错误。

2.删除指定对象

list.remove(object);

如果一个集合里边有多个相同值的对象，调用一次remove只能删除一个，调用多次可以删多次；如果指定的对象不存在，也不会报错哦。

> 如果list里边存储的是integer类型的值，如果我们想按照指定对象的方式删除的话，必须将int类型显示的转成Integer，否则它还是按照下标去删，如果下标不存在就会报错，如果下表存在就会删除掉，但是这种结果可能不是我们想要的，所以使用的时候注意一下
...
public static void main(String[] args) {
  ...
  for (int i = 0; i <list.size(); i++) {
    System.out.println("before:size="+list.size()+",i="+i);
    list.remove(i);
    System.out.println("after:size="+list.size()+",i="+i);
  }
}
```
...
> [1, 2] 
>
> 这是最终删完后list的结果。为毛，我本来想全删的，结果只删了两个。是因为删除的过程中**list.size和数组中元素的位置都在变**，删除了两个元素之后，size=2，不符合条件就没有进行后续的删除工作了。剩余的两个元素的下表也对应的变成了0，1；如果我们此时把上面的i<list.size()变成i<4.那就会下标越界错误。

结论：上面的删除会导致误删，漏删。

##### 使用增强的版的for循环（foreach）删除后break,继续循环
...
> 报这个错误：java.util.ConcurrentModificationException
...
> 代码执行顺序：静态变量、静态代码块(这两部分是按照代码编写顺序，从上到下依次进行)>静态方法>实例变量、代码块（这两部分是按照代码编写顺序，从上到下依次进行）>构造方法>实例方法
...
public class SingtonDemo {
	...
	private static SingtonDemo singtonDemo=new SingtonDemo();//3
	...
	static {
		...
		System.out.println("static:"+count2);
	}
	
	//5
	public SingtonDemo() {
		System.out.println("SingtonDemo:"+count1);
		System.out.println("SingtonDemo:"+count2);
		count1++;
		count2++;
	}
	...
	//4
	{
		System.out.println("代码块"+count1);
		System.out.println("代码块"+count2);
	}
...
### AppClassLoader、ExtClassLoader、URLClassLoader 的关系

1.用户自定义的类是由AppClassLoader加载

2.在双亲委派模式中，ExtClassLoader是AppClassLoader的父亲
...
| 对比点  | get           | post                         |
| ---- | ------------- | ---------------------------- |
| 安全性  | 请求参数会暴露在url里  | 请求参数在request body里边，不能轻易的被发现 |
| 数据大小 | 对数据大小有限制（2kb） | 无限制                          |
| 缓存   | 会被浏览器主动缓存     | 不会主动缓存                       |
| 性能   | 好             | 比get差                        |

###  String  StringBuffer StringBuilder区别

String是不可变的，创建后如果修改的时候，会产生新的对象。
...
如果在不重写Object类的equals方法的情况下，默认equals比较的是两个对象的内存地址，见源码：
...
如果我们现在有一个Student对象，如果他们的名称和年龄都相等的情况下我们认为他就是同一个对象，这时候就需要重写equals方法。
...
> 如果a.equals(b)==true,那么他们的hash一定相等
>
> 如果a b对象的hashcode相等，a.equals(b)不一定相等
...
HashSet实现了set接口，不允许存储重复元素，基于HashMap的key不能重复实现的。
...
//add方法，值存入map的key里边，所有的map的value是一个常量Object对象
...
为变量在每个线程中都创建了一个副本。在没有使用线程池的情况下，线程退出时，系统会回调Thread类的exit方法进行一些清理工作，包括对threadLocals置null，置null的变量会被垃圾回收。
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for (int i = 0; i < 4; i++) {
			executorService.execute(()->{
				System.out.println(Thread.currentThread().getId()+":before"+local.get());
				local.set(local.get()+1);
				System.out.println(Thread.currentThread().getId()+":after"+local.get());
				//local.remove();使用完后及时的清除数据
			});
		}
...
执行代码打印的结果如下：初始化为0，为什么会出现有的before为1的情况，就是因为线程池里边的线程是复用的，所以用完要及时的清除数据，避免这种情况的发生。
...
相同：都是用来解决多线程下相同变量访问冲突问题

不同：synchronized采用了`时间换空间`的方式，只提供一份变量，让不同线程排队访问；而Threadlocal采用了`空间换时间`的方式，为每一个线程提供了一个变量	，因此访问可以不受影响。

**以时间换空间**->即加锁方式，某个区域代码或变量只有一份节省了内存，但是会形成很多线程等待现象，因此浪费了时间而节省了空间。
...
4.CountDownLatch和CyclicBarrier都能够实现线程之间的等待，只不过它们侧重点不同：
...
为了解决死锁，可以给对应的key设置过期时间。

​	1.1由上面的方案产生第二种问题，业务代码还没有执行完，锁失效了，其他的线程就获取到了锁，造成了不可避免的问题；引入锁续期解决方案：在获取锁成功后启动一个定时任务，在指定时间内去延期锁过期时间，当执行完业务代码后释放锁并取消定时任务。代码卸载finally里边，保证代码在任何时候都能执行。

2.锁重入

set的时候值设置为ip+线程id，下次判断如果值相等，重入次数++，释放锁的时候重入次数--，当重入次数为0的时候释放锁。
...
创建临时有序节点，每次判断自己是否是最小的节点，如果是则获取锁成功，反之则获取锁失败，获取锁失败的时候，会watcher前一个节点，当前一个节点释放锁的时候，它会被唤醒
...
* request
  ...
  ​只适用于web程序，每一次http请求都会产生一个新的实例

* session

  ​只适用于web程序，为每一个session创建一个实例

* global session

  ​只有应用在基于portlet的Web应用程序中才有意义，它映射到portlet的global范围的session。如果在普通的基于servlet的Web应用中使了用这个类型的scope，容器会将其作为普通的session类型的scope来对待。
...
### spring boot创建一个scope=request的类

#### 第一次实现方式

@RequestScope 使用这个注解可以轻松实现
